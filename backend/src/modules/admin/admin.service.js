...
export async function upsertSystemSetting(data, userId) {
...
export async function getContestSettings(contestId) {
...
export async function updateContestSettings(contestId, data, userId) {
    ...
    const setting = await prisma.contestSetting.upsert({
        where: {
            contestId,
        },
        update: updateData,
        create: {
            ...
            publishIntervalMinutes: data.publishIntervalMinutes ?? 15,
            firstPrizePercent: data.firstPrizePercent ?? 70,
            secondPrizePercent: data.secondPrizePercent ?? 20,
            thirdPrizePercent: data.thirdPrizePercent ?? 10,
